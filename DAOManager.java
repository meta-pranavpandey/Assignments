...
public class DAOManager {
	...
	/**
	 * This function load the driver as well as connect to driver of vendor
	 * database
	 */
	 ...
	 * 
	 * @param authorName
	 *            is the name of the author whose books we have to fetch from
	 *            5the database.
	 * @return list of the title objects which contains title name and id's.
	 */
	public List<Titles> getBookTitle(String authorName) {
		List<Titles> titleList = new ArrayList<Titles>();

		String query = "SELECT " + "t.title_id,t.title_name from titles t "
		...
		try {
			...
			while (res.next()) {
				Titles tempTitle = new Titles();
				tempTitle.setTitle_Id(res.getString(1));
				tempTitle.setTitle_Name(res.getString(2));
	 ...
	 * 
	 * @param bookName
	 *            is the name of the book required to find out whether issued or
	 *            not
	 * @return boolean true/false
	 */
	public Boolean isBookIssue(String bookName) {

		String query = "SELECT " + "count(*) from book_issue bi "
				+ "INNER JOIN "
	...
	/**
	 * Following function returns the number of books deleted which were not
	 * issued for more than 1 Year.
	 * 
